...
export class ReplenishmentsController {
  ...
  @MessagePattern(REPLENISHMENTS_PATTERN.REPLENISHMENTS_SCAN_SHORTAGE_SNAPSHOT)
  scanShortageSnapshot() {
    return this.replenishmentsService.runShortageSnapshotScan();
  }

  @MessagePattern(REPLENISHMENTS_PATTERN.REPLENISHMENTS_GET_SHORTAGE_SNAPSHOT)
  getShortageSnapshot(@Payload() filter: { warehouseId?: number; scanDate?: string }) {
    return this.replenishmentsService.getShortageSnapshot(filter || {});
  }
}
